...
func (s *JobOnceScheduler) Cancel(key string) {
	s.pluginAPI.LogError("<><> Cancel for key", "key", key)
	...
	job := func() *JobOnce {
		...
		if ok {
			s.pluginAPI.LogError("<><> Cancel for key, found job in activeJobs", "key", key)
			return j
		}
		...
		s.pluginAPI.LogError("<><> Cancel for key, did NOT find job in activeJobs. creating mutex", "key", key)
		...
		s.pluginAPI.LogError("<><> Cancel for key, locking mutex", "key", key)
		mutex.Lock()
		defer mutex.Unlock()

		s.pluginAPI.LogError("<><> Cancel for key, calling KVDelete", "key", key)
	...
	if job != nil {
		s.pluginAPI.LogError("<><> Cancel for key, found job in activeJobs. calling job.Cancel", "key", key)
		job.Cancel()
	}
}
